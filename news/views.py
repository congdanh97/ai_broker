...
def news_detail(request,word):
    ...
    # link = "http://127.0.0.1:8000/{}".format(word) + str(News.objects.get(name=word).rand)  ## For QR Code

    return render(request, 'front/news_detail.html', {'site':site, 'news':news, 'cat':cat,
                                                  'subcat':subcat, 'lastnews':lastnews, 'shownews':shownews,
                                                  'popnews':popnews, 'popnews2':popnews2, 'tag':tag, 'trending':trending,
                                                  'code':code, 'comment':comment, 'cmcount':cmcount,
                                                  'link':link})
...
def news_detail_short(request, pk):
    ...
    link = request.build_absolute_uri()
    original_links = "Bài Gốc : {}".format(link)

    return render(request, 'front/news_detail.html', {'site':site, 'news':news, 'cat':cat, 'subcat':subcat, 'lastnews':lastnews, 'shownews':shownews, 'popnews':popnews, 'popnews2':popnews2, 'tag':tag, 'trending':trending, 'link':link, 'original_links':original_links})
                ...
                if myfile.size < 5000000:## Check File Size ##
                    ...
                    try:
                        b = News(name=newstitle, short_txt=newstxtshort, body_txt=newstxt,
                                 ...
                                 picname=filename, picurl=url, writer=request.user,
                                 catname=newsname, catid=newsid, show=0, tag=tag,
                                 act=0, rand=rand)  # these are the model fields
                        b.save()
