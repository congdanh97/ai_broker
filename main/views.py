...
def home(request):
    try:
        if len(Main.objects.all()) != 0 and len(Trending.objects.all()) != 0 and len(News.objects.all()) != 0:
            site = Main.objects.get(pk=1)

            news = News.objects.filter(act=1).order_by(
                '-pk')  ## for reverse(ordering) need to filter by pk with (-) to get the latest submission first.

            lastnews = News.objects.filter(act=1).order_by(
                '-pk')[:3]  ### This query for last three post

            popnews = News.objects.filter(act=1).order_by(
                '-show')  ### Populer News will be Shown according to view(show)
            ...
            popnews2 = News.objects.filter(act=1).order_by('-show')[
                       :3]  ### 3 Populer News will be Shown according to view(show) in footer section

            trending = Trending.objects.all().order_by('-pk')[
                       :]  ### Trending now will show on top bar(send query from here to naster.html in front)

            random_object = Trending.objects.all()[randint(0, len(
                trending) - 1)]  ## Random Object (For Trending now). I just used it for home page. To show the trending randomly

            subcat = SubCat.objects.all()  ## for SubMenu in the menu bar

            cat = Cat.objects.all()  ## Show categories in footer
            cat_sorted = [cat[3],cat[5],cat[6],cat[1],cat[2],cat[4],cat[0]]  ## Show categories in footer

            news_final = news[len(news) - 1 :]
            ...
            return render(request, 'front/home.html', {'site': site,'cat':cat, 'subcat':subcat})
...
def site_setting(request):
    ...
    if request.method == 'POST':
        ...
        p = Main(name=name, tell=tell, fb=fb, tw=tw, yt=yt,
                 link=link, abouttxt=txt, picurl=picurl, picname=picname, picurl2=picurl2, picname2=picname2)
        p.save()
